...
describe('UserProfile', () => {
  ...
  it('renders displayName', () => {
    const { container } = render(<UserProfile user={baseUser} isOwn={true} />)
  ...
  it('shows follow button for other profiles', () => {
    const { container } = render(<UserProfile user={baseUser} isOwn={false} />)
  ...
  it('shows following state when initialFollowing is true', () => {
    const { container } = render(<UserProfile user={baseUser} isOwn={false} initialFollowing={true} />)
    const followBtn = container.querySelector('.btn-follow')
  ...
  it('unfollows on click when following', async () => {
    ...
    const { container } = render(<UserProfile user={baseUser} isOwn={false} initialFollowing={true} followerCount={5} />)
    const followBtn = container.querySelector('.btn-follow')!
    fireEvent.click(followBtn)
  ...
  it('displays follower count', () => {
    const { container } = render(<UserProfile user={baseUser} isOwn={true} followerCount={10} />)
    expect(container.textContent).toContain('10')
    expect(container.textContent).toContain('フォロワー')
  })

  it('displays following count', () => {
    const { container } = render(<UserProfile user={baseUser} isOwn={true} followingCount={3} />)
